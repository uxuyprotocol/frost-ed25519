// ristretto was taken from https://github.com/gtank/ristretto255/tree/filippo/edwards25519backend with slight modification
package ristretto
...
func TestRistrettoSmallMultiplesTestVectors(t *testing.T) {
	...
	basepointMultiple := NewIdentityElement()
	ristrettoBasepoint := NewGeneratorElement()
	...
	for i := range testVectors {
		...
		_, err = decodedPoint.SetCanonicalBytes(encoding)
		if err != nil {
			t.Fatalf("#%d: could not decode test vector: %v", i, err)
		}
		// Re-encode and check round trips
		roundtripEncoding := decodedPoint.Bytes()
		...
		computedEncoding := basepointMultiple.Bytes()
...
func TestRistrettoFromUniformBytesTestVectors(t *testing.T) {
	...
	for i, input := range inputs {
		...
		_, _ = element.SetUniformBytes(hash[:])
		if encoding := hex.EncodeToString(element.Bytes()); encoding != elements[i] {
...
func TestEquivalentFromUniformBytes(t *testing.T) {
	...
	for i, input := range inputs {
		...
		_, _ = element.SetUniformBytes(h[:])
		if encoding := hex.EncodeToString(element.Bytes()); encoding != expected {
...
func TestScalarSet(t *testing.T) {
	...
	// sc1 <- 1
	_, _ = sc1.SetCanonicalBytes(scOne)
